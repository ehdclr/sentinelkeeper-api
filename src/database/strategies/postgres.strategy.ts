// src/database/strategies/postgres.strategy.ts
import { Injectable } from '@nestjs/common';
import { DatabaseStrategy } from './database-strategy.abstract';
import {
  DatabaseConfig,
  DatabasePool,
  ConnectionTestResult,
} from '../types/database.type';
import {
  SQL_QUERIES,
  REQUIRED_TABLES,
  DEFAULT_ROLES,
  SYSTEM_SETTINGS,
} from '../constants/database.constant';

@Injectable()
export class PostgresStrategy extends DatabaseStrategy {
  get type(): 'postgres' {
    return 'postgres';
  }

  async createConnection(config: DatabaseConfig): Promise<DatabasePool> {
    if (config.type !== 'postgres') {
      throw new Error('Invalid config type for PostgresStrategy');
    }

    try {
      // CommonJS require 방식 사용
      const postgres = require('postgres');
      const { drizzle } = require('drizzle-orm/postgres-js');

      const sql = postgres({
        host: config.host,
        port: config.port,
        username: config.username,
        password: config.password,
        database: config.database,
        ssl: config.ssl,
        max: 10, // 연결 풀 최대 크기
        idle_timeout: 20,
        max_lifetime: 60 * 30,
      });

      const drizzleDb = drizzle(sql);

      const pool: DatabasePool = {
        connection: sql,
        drizzle: drizzleDb,
        isHealthy: true,
        lastUsed: new Date(),
        type: 'postgres',
      };

      this.logger.log('PostgreSQL connection pool created successfully');
      return pool;
    } catch (error) {
      this.logger.error('Failed to create PostgreSQL connection', error);
      throw this.wrapConnectionError(error as Error);
    }
  }

  async testConnection(config: DatabaseConfig): Promise<ConnectionTestResult> {
    if (config.type !== 'postgres') {
      return {
        success: false,
        error: 'Invalid config type for PostgresStrategy',
      };
    }

    try {
      const pool = await this.createConnection(config);

      const result = await pool.connection`SELECT 1 as test`;
      await this.cleanup(pool);

      this.logger.log('PostgreSQL connection test successful');
      return {
        success: true,
        details: {
          type: 'postgres',
          database: config.database,
          host: config.host,
          result: result[0],
        },
      };
    } catch (error) {
      this.logger.error('PostgreSQL connection test failed', error);
      return {
        success: false,
        error: (error as Error).message,
        details: {
          type: 'postgres',
          database: config.database,
          host: config.host,
        },
      };
    }
  }

  async createTables(pool: DatabasePool): Promise<void> {
    try {
      const queries = [
        SQL_QUERIES.POSTGRES.CREATE_USERS,
        SQL_QUERIES.POSTGRES.CREATE_SESSIONS,
        SQL_QUERIES.POSTGRES.CREATE_ROLES,
        SQL_QUERIES.POSTGRES.CREATE_USER_ROLES,
        SQL_QUERIES.POSTGRES.CREATE_SYSTEM_SETTINGS,
        SQL_QUERIES.POSTGRES.CREATE_AGENTS,
      ];

      for (const query of queries) {
        await pool.connection.unsafe(query);
      }

      this.logger.log('PostgreSQL tables created successfully');
    } catch (error) {
      this.logger.error('Failed to create PostgreSQL tables', error);
      throw this.wrapSchemaError('table creation', error as Error);
    }
  }

  async checkTablesExist(pool: DatabasePool): Promise<boolean> {
    try {
      for (const table of REQUIRED_TABLES) {
        const result = await pool.connection`
          SELECT table_name 
          FROM information_schema.tables 
          WHERE table_schema = 'public' AND table_name = ${table}
        `;

        if (result.length === 0) {
          this.logger.warn(`Table '${table}' does not exist`);
          return false;
        }
      }

      this.logger.log('All required PostgreSQL tables exist');
      return true;
    } catch (error) {
      this.logger.warn('Failed to check PostgreSQL table existence', error);
      return false;
    }
  }

  async setupInitialData(pool: DatabasePool): Promise<void> {
    try {
      // 역할 데이터 확인 및 삽입
      const roleResult =
        await pool.connection`SELECT COUNT(*) as count FROM roles`;

      if (parseInt(roleResult[0].count) === 0) {
        for (const role of DEFAULT_ROLES) {
          await pool.connection`
            INSERT INTO roles (name, description) 
            VALUES (${role.name}, ${role.description})
          `;
        }
        this.logger.log('Default roles inserted');
      }

      // 시스템 설정
      const initializedAt = new Date().toISOString();
      const settings = [
        { key: 'app_name', value: SYSTEM_SETTINGS.APP_NAME },
        { key: 'version', value: SYSTEM_SETTINGS.VERSION },
        { key: 'initialized_at', value: initializedAt },
      ];

      for (const setting of settings) {
        await pool.connection`
          INSERT INTO system_settings (key, value) 
          VALUES (${setting.key}, ${setting.value}) 
          ON CONFLICT (key) DO UPDATE SET value = ${setting.value}
        `;
      }

      this.logger.log('PostgreSQL initial data setup completed');
    } catch (error) {
      this.logger.warn('Failed to setup PostgreSQL initial data', error);
      throw this.wrapSchemaError('initial data setup', error as Error);
    }
  }

  async cleanup(pool: DatabasePool): Promise<void> {
    try {
      if (pool.connection && typeof pool.connection.end === 'function') {
        await pool.connection.end();
        this.logger.log('PostgreSQL connection cleaned up');
      }
    } catch (error) {
      this.logger.warn('Failed to cleanup PostgreSQL connection', error);
    }
  }

  async healthCheck(pool: DatabasePool): Promise<boolean> {
    try {
      await pool.connection`SELECT 1`;
      pool.isHealthy = true;
      pool.lastUsed = new Date();
      return true;
    } catch (error) {
      this.logger.warn('PostgreSQL health check failed', error);
      pool.isHealthy = false;
      return false;
    }
  }
}
